import { chainApply, iPoint, traverseNDSquareFrame } from 'matriangle-common'
...
const configConstructor: () => NARSEnvConfig = (): NARSEnvConfig =>
	chainApply(
					...
					behavior: {
						...
						operate: (
							...
							if (operateI >= 0) {
								...
								console.warn(
									`未知的操作「(${selfConfig.NAL.op_output(
										op
									)})」`
								)
							// 没执行⇒没成功
		...
		// 链式应用
		modifyPlotOfEnvConfig
	)
// * 魔改图表，使之并行支持「多NARS智能体」数据曲线
function modifyPlotOfEnvConfig(config: NARSEnvConfig): NARSEnvConfig {
