...
const plot = new Plot(
	document.getElementById('plot'),
	{
		...
		legend: {
			data: ['成功率', '自主成功率', '教学成功率', '激活率'] // 直接是名字列表
		},
		xAxis: {
			type: 'category',
			boundaryGap: false,
...
/** 格式约定：「图表」+待显示数据如{x: XXX, y:XXX} 其中的「x」将被解码成x坐标 */
const isPlotSignal = text => text.startsWith('图表')
...
function resetScreenWS(force = false /* 默认为假，留给「自动重连」调用 */) {
	...
	if (socketScreen) {
		...
		socketScreen.onmessage = event => {
			...
			else if (isPlotSignal(dataS)) setPlot(dataS.slice(2)) // * 新：图表
 ...
 * 刷新图表
 *
 * ! 使用「JSON文本解析后的对象」中的「x」属性值作为「横轴属性」
 *
 * @param {string} textJSON JSON文本
 */
function setPlot(textJSON) {
	let data = JSON.parse(textJSON)
	plot && plot.addData(data?.x, data)
}
