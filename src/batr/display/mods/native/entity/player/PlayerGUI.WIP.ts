...
export default class PlayerGUI implements IShapeContainer {
	...
	public get logicalX(): number {
		return real2Logical(this.x)
	}

	/** 从「显示坐标」获取自身「逻辑坐标」 */ // ? 是否其实只是一个「单向过程」？
	public get logicalY(): number {
		return real2Logical(this.y)
	}
	...
	public getVisibleCD(): boolean {
		return this._CDBarFrame.visible
	}
	public getVisibleCharge(): boolean {
		return this._chargeBarFrame.visible
	}
	public getVisibleExperience(): boolean {
	...
	protected sortUnderBars(): void {
		let sortCD: uint = 0,
			sortCharge: uint = 0 /*,sortExperience:uint=0*/
