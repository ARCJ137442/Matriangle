 ...
 * * 可能在地图中任何位置产生的（非格点实体）
 * * 有相对固定的生命周期lifespan的
 * * 相对活跃但不受影响游戏逻辑的(不会接收游戏回调的钩子)
 * (轻量级)实体
 * ```
 * ? 参考Minecraft的「粒子效果」或许「独立出去」也值得考量
 */
export default abstract class Effect extends Entity implements IEntityDisplayable, IEntityShortLived, IEntityFixedLived, IEntityOutGrid, IEntityActiveLite {
