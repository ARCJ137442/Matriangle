...
export default interface IBatrGame {
	...
	//======Entity Getters======//
	get playerContainer(): Sprite {
		return this._playerContainer;
	}

	get projectileContainer(): Sprite {
		return this._projectileContainer;
	}

	get bonusBoxContainer(): Sprite {
		return this._bonusBoxContainer;
	}

	get playerGUIContainer(): Sprite {
		return this._playerGUIContainer;
	}

	get effectContainerBottom(): Sprite {
		return this._effectContainerBottom;
	}

	get effectContainerTop(): Sprite {
...
testGameEnd(force: boolean = false): void {
	...
	if (winners.length < 1) {
		...
		return new FixedI18nText(
			...
			winners.map(
				player => player.customName // ! 假定不可能存在空值
			).join(',')
		);
	}
}
...
toolCreateExplode(x: number, y: number, finalRadius: number,
...
laserHurtPlayers(laser: LaserBasic): void {
	// Set Variables
...
hitTestOfPlayers(...players): boolean {
	// Transform
