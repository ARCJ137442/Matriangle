...
import { TICK_TIME_MS } from "./GlobalGameVariables";
...
export default class Matrix_V1 implements IBatrMatrix {
	...
	public initByRule(rule: GameRule, becomeActive: boolean = false): boolean {
		// Check
		if (this._isLoaded)
			return false;
		// Update
	...
	public restart(rule: GameRule, becomeActive: boolean = false): void {
		this.reset();
		this.initByRule(rule, becomeActive);
	}

	public forceStart(rule: GameRule, becomeActive: boolean = false): boolean {
