 ...
 * * 不负责「世界运行」以外的事务
 *   * 有关「游戏注册信息」的处理：如「实体类型列表」
 *   * 有关「游戏呈现/显示」的功能，如「国际化文本」「显示端」
 * 
 * TODO: 是否要把「实体系统」「地图」这两遍历内置（封装），仅对外部保留必要接口（以便后续显示更新）？
 * 
 * 可以基于此假设多种「端侧」
 * * 服务端：游戏在实际上运作于此上，用于实际运行代码
 * * 客户端：类似Minecraft，游戏只有一部分机制运行于此上（仅为流畅考虑）
...
export default interface IBatrMatrix {
	...
	*/
	 ...
	 * * 实现方法：在最前面附带「游戏母体」以让函数完全独立于任何一个类
	 * 
	 * !【2023-10-02 22:37:20】现已全部弃用：不再把「具体机制」内置于此，改为在「原生游戏机制」中使用
	 */
	...
	//======Entity Functions======// TODO: 计划外置到「原生游戏机制」中
	...
	//======Hook Functions======// TODO: 计划外置到「原生游戏机制」中
