...
import { IEntityActive, i_shortLive } from '../../../api/entity/EntityInterfaces'
import IMatrix from '../../../main/IMatrix'

/**
 * 这个类其实有一部分是在实验JS「动态类型特性」中的「任意属性均可黑」「实例方法亦可改」的特性
 */
export default class ProgramMerovingian extends MatrixProgram implements IEntityActive {
	/** 标签 */
	 ...
	 * * 逻辑：只要不是「短命的」就行
	 */
	public static readonly DEFAULT_WANTS: (host: IMatrix, e: Entity) => boolean = (host: IMatrix, e: Entity): boolean =>
		!i_shortLive(e)
	 ...
	 * * 逻辑：母体内实体数 < 5
	 */
	public static readonly DEFAULT_PUSH_BACKS: (host: IMatrix, e: Entity) => boolean = (
		host: IMatrix,
		e: Entity
	): boolean => host.entities.length < 5
	...
	constructor(
		 ...
		 * 决定这个程序「私藏」的偏好
		 */
		public readonly wants: (host: IMatrix, e: Entity) => boolean = ProgramMerovingian.DEFAULT_WANTS,
		/**
		 * 决定这个程序「放回到某个母体」的偏好
		 */
		public readonly pushBacks: (host: IMatrix, e: Entity) => boolean = ProgramMerovingian.DEFAULT_PUSH_BACKS
	) {
	...
	onTick(host: IMatrix): void {
		// 有私藏⇒检查第一个
		if (this.privatePossessions.length > 0 && this.pushBacks(host, this.privatePossessions[0])) {
			// 重新激活
	...
	public hack(matrix: IMatrix): void {
		...
		// 篡改函数
		const oldF: (entity: Entity) => boolean = matrix.removeEntity.bind(matrix)
		;(matrix.removeEntity as unknown) = (entity: Entity): boolean => {
			...
			// 「应该被删除」但「自己想要」⇒私藏它
			if (oldResult) if (this.wants(matrix, entity)) this.privatePossessions.push(entity)
			return oldResult
		}
	}
}
