...
import { TICK_TIME_MS } from "../../../main/GlobalWorldVariables";
...
export default class Matrix_V1 implements IMatrix {
	...
	public getAlivePlayers(): IPlayer[] {
		let result: IPlayer[] = new Array<PlayerBatr>();
		for (let player of getPlayers(this)) {
			if (player === null)
				continue;
	...
	public transformMap(destination: IMap = null): void {
		...
		for (let player of players) {
			if (player instanceof PlayerBatr)
				(player as PlayerBatr).onMapTransform();
		}
		// Stat
	...
	protected createPlayer(x: int, y: int, id: uint, team: PlayerTeam, isActive: boolean = true): IPlayer {
		return new PlayerBatr(this, x, y, team, id, isActive);
	}
	...
	public hitTestOfPlayers(...players): boolean {
		// Transform
		let _pv: IPlayer[] = new PlayerBatr[];

		let p: any;

		for (p of players) {
			if (p instanceof PlayerBatr) {
				_pv.push(p as PlayerBatr);
			}
		}
		// Test
		for (let p1 of _pv) {
	...
	public getHitPlayers(x: number, y: number): IPlayer[] {
		// Set
		let returnV: IPlayer[] = new PlayerBatr[];
