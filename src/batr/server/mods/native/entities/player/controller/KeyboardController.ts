...
import { DEFAULT_PLAYER_CONTROL_KEYS, NativeControlKeyConfig } from '../../../../batr/mechanics/BatrMatrixMechanics'
...
export default class KeyboardController extends MatrixController {
	...
	public constructor(label: string, owner: IPlayer, listener: KeyListener) {
		super(label)
		this._owner = owner
	...
	public get someKeyDown(): boolean {
		return (
			this.isPress_Up || this.isPress_Down || this.isPress_Left || this.isPress_Right || this.isPress_Use /*||
	...
	public get someMoveKeyDown(): boolean {
		return this.isPress_Up || this.isPress_Down || this.isPress_Left || this.isPress_Right
	}
	/*
	...
	public set isUsing(turn: boolean) {
	...
	public dealKeyControl(): void {
		if (!this._owner.isActive || this._owner.isRespawning) return
		if (this.someKeyDown) {
			// Move
