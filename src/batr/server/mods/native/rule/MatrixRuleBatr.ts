...
export default class MatrixRuleBatr extends MatrixRule_V1 {
	...
	public set playerCount(value: uint) {
		if (MatrixRuleBatr.preUpdateVariable(this, MatrixRuleBatr.key_playerCount, this._playerCount, value))
	...
	public set AICount(value: uint) {
		if (MatrixRuleBatr.preUpdateVariable(this, MatrixRuleBatr.key_AICount, this._AICount, value))
			this._AICount = value
	}
	...
	public static readonly key_playerTeams: key = fastAddJSObjectifyMapProperty_dash(
		...
		Array<PlayerTeam>,
		// * 保存玩家数组：一一映射存储
		(arr: PlayerTeam[]): JSObject[] => arr.map((pt: PlayerTeam): JSObject => uniSaveJSObject(pt, {})),
		// * 加载玩家数组：一一映射加载
	...
	protected _allowPlayerChangeTeam: boolean = MatrixRuleBatr.d_allowPlayerChangeTeam
	/**
	 * 是否允许玩家（使用一般方式）改变其队伍
	 */
	...
	public set defaultHP(value: uint) {
		if (MatrixRuleBatr.preUpdateVariable(this, MatrixRuleBatr.key_defaultHP, this._defaultHP, value))
	...
	public set defaultMaxHP(value: uint) {
		if (MatrixRuleBatr.preUpdateVariable(this, MatrixRuleBatr.key_defaultMaxHP, this._defaultMaxHP, value))
	...
	public set remainLivesPlayer(value: int) {
		if (
			MatrixRuleBatr.preUpdateVariable(this, MatrixRuleBatr.key_remainLivesPlayer, this._remainLivesPlayer, value)
		)
	...
	public set remainLivesAI(value: int) {
		if (MatrixRuleBatr.preUpdateVariable(this, MatrixRuleBatr.key_remainLivesAI, this._remainLivesAI, value))
	...
	protected readonly _deadPlayerMoveTo: iPoint = MatrixRuleBatr.d_deadPlayerMoveTo.copy()
	/**
	 * 玩家死亡后移动到的地点
	 */
	...
	public set recordPlayerStats(value: boolean) {
		if (
			MatrixRuleBatr.preUpdateVariable(this, MatrixRuleBatr.key_recordPlayerStats, this._recordPlayerStats, value)
		)
	...
	public set bonusBoxMaxCount(value: int) {
		if (MatrixRuleBatr.preUpdateVariable(this, MatrixRuleBatr.key_bonusBoxMaxCount, this._bonusBoxMaxCount, value))
	...
	public static readonly key_bonusTypePotentials: key = fastAddJSObjectifyMapProperty_dash(
		...
		(m: Map<BonusType, number>): JSObject =>
			mapSaveJSObject(m, (type: BonusType, value: number): [JSObjectValue, JSObjectValue] => [type, value]),
		(v: JSObjectValue): Map<BonusType, number> => {
			if (v instanceof Map) return v
			return mapLoadJSObject(v as JSObject, (bonusType: unknown, weight: unknown): [BonusType, number] => [
				String(bonusType),
				Number(weight),
			])
		},
	...
	protected _bonusBoxSpawnAfterPlayerDeath: boolean = MatrixRuleBatr.d_bonusBoxSpawnAfterPlayerDeath
	...
	protected _bonusBuffAdditionAmount: uint = MatrixRuleBatr.d_bonusBuffAdditionAmount
	...
	protected _bonusMaxHPAdditionAmount: uint = MatrixRuleBatr.d_bonusMaxHPAdditionAmount
	...
	public static readonly key_mapRandomPotentials: key = fastAddJSObjectifyMapProperty_dash2(
		...
		(m: Map<IMap, number>): JSObject =>
			mapSaveJSObject(m, (map: IMap, value: number): [JSObjectValue, JSObjectValue] => [
				uniSaveJSObject(map, {}),
				value,
			]),
		(v: JSObjectValue): Map<IMap, number> => {
			if (v instanceof Map) return v
			return mapLoadJSObject(v as JSObject, (mapJSO: JSObject, weight: unknown): [IMap, number] => [
				uniLoadJSObject(
	...
	protected _mapRandomPotentials: Map<IMap, number> = MatrixRuleBatr.d_mapRandomPotentials
	/**
	 * 格式：地图→权重
	 * * 只在「世界加载」阶段被注册使用。不会在这里注入一丝默认值
	 * * 默认是空映射
	...
	public set initialMap(value: IMap | null) {
		if (MatrixRuleBatr.preUpdateVariable(this, MatrixRuleBatr.key_initialMap, this._initialMap, value))
	...
	public set mapTransformTime(value: uint) {
		if (MatrixRuleBatr.preUpdateVariable(this, MatrixRuleBatr.key_mapTransformTime, this._mapTransformTime, value))
	...
	public set enabledTools(value: Tool[]) {
		if (MatrixRuleBatr.preUpdateVariable(this, MatrixRuleBatr.key_enabledTools, this._enabledTools, value))
	...
	protected _defaultTool: Tool | 'u-random' | 'c-random' = MatrixRuleBatr.d_defaultTool
	public get defaultTool(): Tool | 'u-random' | 'c-random' {
		return this._defaultTool
	}
	public set defaultTool(value: Tool | 'u-random' | 'c-random') {
		if (MatrixRuleBatr.preUpdateVariable(this, MatrixRuleBatr.key_defaultTool, this._defaultTool, value))
	...
	public set maxLaserLength(value: uint) {
		if (MatrixRuleBatr.preUpdateVariable(this, MatrixRuleBatr.key_maxLaserLength, this._maxLaserLength, value))
	...
	public set bulletMaxLife(value: uint) {
		if (MatrixRuleBatr.preUpdateVariable(this, MatrixRuleBatr.key_bulletMaxLife, this._bulletMaxLife, value))
	...
	protected _allowLaserThroughAllBlock: boolean = MatrixRuleBatr.d_allowLaserThroughAllBlock
	...
	public set toolsNoCD(value: boolean) {
		if (MatrixRuleBatr.preUpdateVariable(this, MatrixRuleBatr.key_toolsNoCD, this._toolsNoCD, value))
	...
	public set allowTeamVictory(value: boolean) {
		if (MatrixRuleBatr.preUpdateVariable(this, MatrixRuleBatr.key_allowTeamVictory, this._allowTeamVictory, value))
