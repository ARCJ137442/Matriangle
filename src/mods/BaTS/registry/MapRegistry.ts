...
export module BatrDefaultMaps {
	...
	const BEDROCK: Block = BatrBlockPrototypes.BEDROCK.softCopy()
	const WALL: Block = BatrBlockPrototypes.WALL.softCopy()
	const WATER: Block = BatrBlockPrototypes.WATER.softCopy()
	const GLASS: Block = BatrBlockPrototypes.GLASS.softCopy()
	const METAL: Block = BatrBlockPrototypes.METAL.softCopy()
	const MOVEABLE_WALL: Block = BatrBlockPrototypes.MOVEABLE_WALL.softCopy()
	const X_TRAP_HURT: Block = BatrBlockPrototypes.X_TRAP_HURT.softCopy()
	const X_TRAP_KILL: Block = BatrBlockPrototypes.X_TRAP_KILL.softCopy()
	const X_TRAP_ROTATE: Block = BatrBlockPrototypes.X_TRAP_ROTATE.softCopy()
	const COLOR_SPAWNER: Block = BatrBlockPrototypes.COLOR_SPAWNER.softCopy()
	const LASER_TRAP: Block = BatrBlockPrototypes.LASER_TRAP.softCopy()
	const SUPPLY_POINT: Block = BatrBlockPrototypes.SUPPLY_POINT.softCopy()
	const GATE_OPEN: Block = BatrBlockPrototypes.GATE_OPEN.softCopy() //.setState({ open: true }); // !【2023-10-07 21:55:51】现在直接用原型对象，不再使用「链式操作」动态设置键值对
	...
	STORAGE_5.generatorF = (storage: IMapStorage): IMapStorage => {
		...
		let randNum: int = 24 + randInt(47),
			randPrototype: Block
		let iP: iPoint
		while (--randNum > 0) {
			iP = storage.randomPoint
			if (storage.getBlockID(iP) == BatrBlockIDs.BEDROCK) {
				storage.setVoid(iP)
			} /*
				...
				randPrototype = randomIn(ALL_BATR_BLOCKS)
	...
	{
		setBlock(STORAGE_6, 3, 3, BatrBlockPrototypes.COLOR_SPAWNER.copy())
		setBlock(STORAGE_6, 3, 20, BatrBlockPrototypes.COLOR_SPAWNER.copy())
		setBlock(STORAGE_6, 20, 3, BatrBlockPrototypes.COLOR_SPAWNER.copy())
		setBlock(STORAGE_6, 20, 20, BatrBlockPrototypes.COLOR_SPAWNER.copy())

		// 1
		setBlock(STORAGE_6, 20, 9, BatrBlockPrototypes.COLOR_SPAWNER.copy())
		setBlock(STORAGE_6, 20, 14, BatrBlockPrototypes.COLOR_SPAWNER.copy())

		// x+
		setBlock(STORAGE_6, 3, 9, BatrBlockPrototypes.COLOR_SPAWNER.copy())
		setBlock(STORAGE_6, 3, 14, BatrBlockPrototypes.COLOR_SPAWNER.copy())

		// x-
		setBlock(STORAGE_6, 9, 20, BatrBlockPrototypes.COLOR_SPAWNER.copy())
		setBlock(STORAGE_6, 14, 20, BatrBlockPrototypes.COLOR_SPAWNER.copy())

		// y+
		setBlock(STORAGE_6, 9, 3, BatrBlockPrototypes.COLOR_SPAWNER.copy())
		setBlock(STORAGE_6, 14, 3, BatrBlockPrototypes.COLOR_SPAWNER.copy())
	...
	STORAGE_8.copyFrom(STORAGE_FRAME)
	{
		...
		// down
		drawLaserTrapDownPillar(STORAGE_8, 4, WALL, GLASS, BEDROCK, LASER_TRAP)
		drawLaserTrapDownPillar(STORAGE_8, 14, WALL, GLASS, BEDROCK, LASER_TRAP)
		// up
		drawLaserTrapUpPillar(STORAGE_8, 9, WALL, GLASS, BEDROCK, LASER_TRAP)
		drawLaserTrapUpPillar(STORAGE_8, 19, WALL, GLASS, BEDROCK, LASER_TRAP)
	}
	//====Map 9====//
