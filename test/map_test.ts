...
const assert_equal: (v1: any, v2: any) => void = (v1: any, v2: any): void =>
	assert(v1 === v2, `Assertion failed: ${v1} !== ${v2}`)
...
function 地图读取测试(): void {
	...
	const ml: IMapLogic = m,
		ms: IMapStorage = m.storage
...
;(function 多维点性能遍历测试(s: int, e: int): void {
	function sum_2d(x: int, y: int): int {
		return x + y
	}
	function sum(p: iPoint): int {
		return p.x + p.y
	}
	...
	time('2d')
	for (let x = s; x < e; x++) for (let y = s; y < e; y++) sum_2d(x, y)
	timeEnd('2d')
