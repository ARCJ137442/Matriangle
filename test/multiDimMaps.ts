...
for (let i: uint = DIM_MIN; i <= DIM_MAX; i++) {
	...
	if (DIM_MAXs.checkInvalid() || DIM_MINs.checkInvalid())
		throw new Error(`Invalid dimension size: ${DIM_MINs.toString()} ~ ${DIM_MAXs.toString()}`)
	// 存储
	...
	if (i > 1)
		// 一维就不要添堵了
		mss.setBlock(new iPoint().copyFrom(DIM_MAXs).fill(0), NativeBlockPrototypes.COLORED.softCopy())
	// 生成
	map = new Map_V1(`维度测试_${i}维@${mss.size.join('x')}`, mss, new iPoint().copyFromArgs(...mss.size))
	// 注册
